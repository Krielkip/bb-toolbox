...
final class BB_Addon_Toolbox
{
    ...
    static public function save_toolbox_settings($settings = array(), $post_id = '')
    {
        ...
        if (is_post_type_hierarchical($post->post_type)) {
            if (($settings['post_parent'] !== 'default' && $post->post_parent === 0) || ($settings['post_parent'] === 'default' || $settings['post_parent'] !== $post->post_parent)) {
                $postarr['post_parent'] = $settings['post_parent'] === 'default' ? 0 : $settings['post_parent'];
                $newLink = true;
            }
        }
    ...
    {
        if (self::has_seo_plugin(array('classes' => array('All_in_One_SEO_Pack', 'All_in_One_SEO_Pack_p')))) {
            // All in one SEO : aiosp_title + aiosp_description
            $meta_title_field = '_aioseop_title';
            $meta_description_field = '_aioseop_description';
        } else if (self::has_seo_plugin(array('classes' => array('wpSEO'), 'constants' => array('WPSEO_VERSION')))) {
            //  WordPress SEO
        ...
        return array($meta_title, $meta_description, $meta_title_field, $meta_description_field);
    }
    ...
    static public function get_dropdown_pages($args = '')
    {
        ...
        if (!empty($pages)) {
            ...
            foreach ($pages as $page) {
                ...
                } else if ($depth !== 0 && $page->post_parent === 0) {
                    $depth = 0;
                    $parent = 0;
                }
